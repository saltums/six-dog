(() => {
  ...
  async function load() {
    ...
    groups = window.SixDogEditor ? await window.SixDogEditor.fetchEventGroupsPublic() : {};
  ...
  async function unlink(variant) {
    const next = Object.assign({}, groups);
    delete next[variant];
    mergeStatus.textContent = "更新中...";
    try {
      await window.SixDogEditor.saveEventGroups(next);
      groups = next;
