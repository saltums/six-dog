...
- `docs/data/tweets.json` に `{event_date, tweet_id}` の対応表を保持(写真そのものはこちらでは複製せず、X公式の [oEmbed API](https://publish.x.com) 経由でその場に埋め込み表示している)
- 現時点では2015年・2016年8〜12月・2018年11〜12月の一部のみ収録(Xの検索は2014年以前の投稿を正しく返さないことがあるため、確認しながら手作業で収集している)。`tweets.json` に同じ形式で行を追加すれば他の日付も追加できる
