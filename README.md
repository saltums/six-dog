...
## イベント名のグルーピング(`docs/event-groups.html`)

「Hello→ vol.130」「Hello→ vol.131」のような同じ企画シリーズの別回を、**公演ごとの表示は元のタイトルのまま残しつつ**、カレンダー検索でだけグループ名でもまとめて見つけられるようにする管理画面。ナビの「イベント管理」から開く。仕組みはアーティスト名の統合と同じ(Levenshtein距離による類似表記の絞り込みなど)だが、**ログイン・トークン不要で誰でもグループ化・解除できる**点が異なる。

- バックエンドはSupabaseの `event_title_groups` テーブル(列: `id`, `variant_title`, `group_name`, `submitted_by`, `submitted_at`)。Row Level Securityで「閲覧・投稿・更新・削除は誰でも可」を設定
- 一覧から複数のタイトルにチェックを入れ、グループ名を入力して「グループにする」を押すとグループ化される。各メンバーは「✕ 解除」でいつでも解除できる
- グループ名はイベントデータ側の文字列自体は書き換えない。公演詳細には引き続き元のタイトルが表示される
...
## 当日の告知ツイート埋め込み(`docs/tweets.js`)

SiX-DOG公式Xアカウント([@shinsakae6dog](https://x.com/shinsakae6dog))が公演当日に投稿していた告知ポストを、該当する公演の詳細に埋め込み表示する。当日の看板やフライヤーの写真が添付されていることが多く、出演者の当日変更などスクレイピングだけでは分からない情報を補完できる。

- 対応表はSupabaseの `event_tweets` テーブル(列: `id`, `event_date`, `tweet_id`, `submitted_by`, `submitted_at`)に保持。写真そのものはこちらでは複製せず、X公式の [oEmbed API](https://publish.x.com) 経由でその場に埋め込み表示している
- 動画URL投稿と同様に**ログイン・トークン不要で誰でも紐付け・解除ができる**。公演詳細の投稿URL入力欄にXの投稿URL(`.../status/12345...`)を貼って「紐付ける」、埋め込み表示の「✕ 解除」で取り消せる。Row Level Securityで「閲覧・投稿・削除は誰でも可」を設定(videosテーブルと違い、間違った紐付けを誰でも直せるよう削除も許可している)
- 現時点では2015年・2016年8〜12月・2018年11〜12月の一部を初期データとして投入済み(Xの検索は2014年以前の投稿を正しく返さないことがあるため、確認しながら手作業で収集した)。それ以外の日付は上記の投稿フォームから誰でも追加できる
- 対応するツイートが無い日付には何も表示されない(エラーにはならない)
